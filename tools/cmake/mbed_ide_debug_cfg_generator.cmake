...
")
	...
	function(mbed_finalize_ide_debug_configurations)
		...
		# CLion has no tasks.json equivalent, so emit a "Reset MCU" Shell Script run
		# configuration if the upload method defines a reset command.
		if(MBED_UPLOAD_RESET_COMMAND)

			set(CLION_RUN_CONF_DIR ${CMAKE_SOURCE_DIR}/.idea/runConfigurations)
			file(MAKE_DIRECTORY ${CLION_RUN_CONF_DIR})

			# Split executable from args and build a shell command line.
			list(GET MBED_UPLOAD_RESET_COMMAND 0 RESET_EXECUTABLE)
			list(SUBLIST MBED_UPLOAD_RESET_COMMAND 1 -1 RESET_ARGS)

			# Quote the executable; PowerShell needs '&' to invoke a quoted path.
			if(CMAKE_HOST_WIN32)
				set(RESET_CMD_STR "& \"${RESET_EXECUTABLE}\"")
				set(RESET_INTERPRETER "powershell.exe")
			else()
				set(RESET_CMD_STR "\"${RESET_EXECUTABLE}\"")
				set(RESET_INTERPRETER "/bin/sh")
			endif()

			foreach(ELEMENT ${RESET_ARGS})
				if("${ELEMENT}" MATCHES " ")
					string(APPEND RESET_CMD_STR " \"${ELEMENT}\"")
				else()
					string(APPEND RESET_CMD_STR " ${ELEMENT}")
				endif()
			endforeach()

			# XML-escape the command string (& first, then quotes).
			string(REPLACE "&" "&amp;" RESET_CMD_STR "${RESET_CMD_STR}")
			string(REPLACE "\"" "&quot;" RESET_CMD_STR "${RESET_CMD_STR}")

			# Generate a Shell Script run configuration that runs the command inline.
			file(GENERATE OUTPUT "${CLION_RUN_CONF_DIR}/Reset MCU.xml" CONTENT
"<!-- Autogenerated by Mbed OS.  Do not edit! -->
<component name=\"ProjectRunConfigurationManager\">
  <configuration default=\"false\" name=\"Reset MCU\" type=\"ShConfigurationType\">
    <option name=\"SCRIPT_TEXT\" value=\"${RESET_CMD_STR}\" />
    ...
    <option name=\"INTERPRETER_PATH\" value=\"${RESET_INTERPRETER}\" />
